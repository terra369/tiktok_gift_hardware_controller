...
class SerialGiftProcessor:
    def __init__(self, port: str, baud_rate: int, ready_signal: str, gift_command: str,
                 gift_queue: asyncio.Queue, process_cooldown: float):
        """
    ...
    def _initialize_serial(self) -> bool:
        """シリアルポートを初期化して接続します。"""
        try:
            logger.info(f"シリアルポート {self.port} (ボーレート: {self.baud_rate}) に接続試行中...")
            self.serial_conn = serial.Serial(self.port, self.baud_rate, timeout=1)
            time.sleep(2) # Arduinoのリセット直後の安定化のため
            logger.info(f"シリアルポート {self.port} に接続成功しました。")
            return True
        except serial.SerialException as e:
            logger.error(f"シリアルポート {self.port} への接続に失敗しました: {e}", exc_info=True)
    ...
    def _process_gifts_loop(self):
        ...
        if not self._initialize_serial():
            logger.error("シリアルポートの初期化に失敗したため、ギフト処理スレッドを終了します。")
            return
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if self.serial_conn and self.serial_conn.is_open and can_process_gift:
                    ...
                    if self.serial_conn.in_waiting > 0:
                        ...
                        if line == self.ready_signal:
                            logger.info(f"Arduinoから '{self.ready_signal}' 信号を受信しました。"
                            ...
                            logger.info(f"ギフトコマンド '{self.gift_command}' をArduinoに送信しました。ギフト: {gift_info['name']}")
                        elif line: # Ready信号ではないが何か受信した場合
                            logger.warning(f"Arduinoから予期しない信号を受信: '{line}' (期待値: '{self.ready_signal}')")
                    else: # in_waiting == 0
                ...
                logger.debug("シリアル読み取りタイムアウト") # 通常動作の一部なのでdebugレベル
            except serial.SerialException as e:
                ...
                if self._reconnect_serial():
                    ...
                    logger.error("シリアル再接続に失敗。スレッドを終了する可能性があります。5秒待機します。")
                    # 深刻なエラーの場合、スレッドを終了させるか、より堅牢なリトライループが必要
                    self._stop_event.wait(5)
            except asyncio.QueueEmpty: # get_nowaitで発生しうるが、can_process_giftでチェック済み
                pass # キューが空なのは通常状態
            except Exception as e:
    ...
    def _reconnect_serial(self) -> bool:
        ...
        if self.serial_conn and self.serial_conn.is_open:
            try:
                ...
                logger.error(f"既存のシリアル接続クローズ中にエラー: {e}", exc_info=True)
    ...
    def start_processing(self):
        ...
        self._processing_thread = threading.Thread(target=self._process_gifts_loop, daemon=True)
    ...
    def stop_processing(self):
        ...
        if self._processing_thread and self._processing_thread.is_alive():
            ...
            logger.info("ギフト処理スレッドは実行されていなかったか、既に終了しています。")
